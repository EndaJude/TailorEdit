...
from utils.dataset import ControlDataset
from utils.arguments import parse_args
from utils.utils import show_fid, update_fid
...
def main():
    ...
    controlnet = ControlNetModel.from_unet(unet, conditioning_channels=18)
    with torch.no_grad():
        new_conv_in = nn.Conv2d(
            4, controlnet.conv_in.out_channels, controlnet.conv_in.kernel_size, controlnet.conv_in.stride, controlnet.conv_in.padding
        )
        new_conv_in.weight.zero_()
        new_conv_in.weight[:, :4, :, :].copy_(unet.conv_in.weight[:, :4, :, :])
        controlnet.conv_in = new_conv_in
        controlnet.config.in_channels = 4
    ...
    update_params = [param for param in controlnet.parameters()]
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("ControlNet", config=vars(args))
