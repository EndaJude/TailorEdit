...
from utils.utils import show_fid, update_fid, CombinedDataset, SequentialDatasetSampler
...
def log_validation(
    ...
    with autocast_ctx:
        for idx, batch in enumerate(dataloader):
            ...
            activate_lora_expert = batch["type"][0]
            edited_image = pipeline(
                                ...
                                activate_lora_expert=activate_lora_expert
                            ).images
...
def main():
    args = parse_args()
    args.edit_localization = True
    args.pretrained_controlnet_path = "./checkpoints/ControlNetModel_18c"
    ...
    if args.use_lora:
        ...
        unet = inject_adapter_in_model(lora_config, unet, 'alt')
        unet = inject_adapter_in_model(lora_config, unet, 'add')
        unet = inject_adapter_in_model(lora_config, unet, 'rem')
        init_moe_in_lora(unet)

        rep_weights = torch.load("./checkpoints/onlyLoRA/lora_rep.pth")
        chg_weights = torch.load("./checkpoints/onlyLoRA/lora_alt.pth")
        add_weights = torch.load("./checkpoints/onlyLoRA/lora_add.pth")
        del_weights = torch.load("./checkpoints/onlyLoRA/lora_rem.pth")
        unet.load_state_dict(rep_weights, strict=False)
        unet.load_state_dict(chg_weights, strict=False)
        unet.load_state_dict(add_weights, strict=False)
        unet.load_state_dict(del_weights, strict=False)
    ...
    from modules.unet import set_unet_weight_requires_grad
    set_unet_weight_requires_grad(unet)
    ...
    opt_states_dict = torch.load("test_moe_ft/checkpoint-90000/optimizer.bin")
    optimizer.load_state_dict(opt_states_dict)
    ...
    if args.dataset_name is not None:
        ...
        train_dataset_replace = InstructFashionDataset(
        ...
        val_dataset_replace = InstructFashionDataset(
            path=args.dataset_name,
            task="rep",
            split="val",
            min_resize_res=args.resolution,
        ...
        train_dataset_alterate = InstructFashionDataset(
            path=args.dataset_name,
            task="alt",
            split="train",
            min_resize_res=args.resolution,
        ...
        val_dataset_alterate = InstructFashionDataset(
            path=args.dataset_name,
            task="alt",
            split="val",
            min_resize_res=args.resolution,
        ...
        train_dataset_add = InstructFashionDataset(
            path=args.dataset_name,
            task="add",
            split="train",
            min_resize_res=args.resolution,
        ...
        val_dataset_add = InstructFashionDataset(
            path=args.dataset_name,
            task="add",
            split="val",
            min_resize_res=args.resolution,
        ...
        train_dataset_remove = InstructFashionDataset(
            path=args.dataset_name,
            task="rem",
            split="train",
            min_resize_res=args.resolution,
        ...
        val_dataset_remove = InstructFashionDataset(
            path=args.dataset_name,
            task="rem",
            split="val",
            min_resize_res=args.resolution,
    ...
    def collate_fn(examples):
        ...
        ems["type"] = [example["type"] for example in examples]
    ...
    train_dataset = CombinedDataset([train_dataset_replace, train_dataset_alterate, train_dataset_add, train_dataset_remove])
    val_dataset = CombinedDataset([val_dataset_replace, val_dataset_alterate, val_dataset_add, val_dataset_remove])
    train_sampler = SequentialDatasetSampler([len(train_dataset_replace), len(train_dataset_alterate), len(train_dataset_add), len(train_dataset_remove)], batch_size=args.train_batch_size)
    val_sampler = SequentialDatasetSampler([len(val_dataset_replace), len(val_dataset_alterate), len(val_dataset_add), len(val_dataset_remove)], batch_size=1)
